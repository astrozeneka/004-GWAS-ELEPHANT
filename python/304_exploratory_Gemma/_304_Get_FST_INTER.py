...
if __name__ == '__main__':
    files = {
        "hair": "../../data/result_stacks/hair/populations.phistats.tsv",
        "tusk": "../../data/result_stacks/tusk/populations.phistats.tsv"
    }
    for file in files:
        fst_data = open(files[file]).read().strip().split("\n")[4:]
        fst_data = [a.split("\t") for a in fst_data]
        fst_data = [[a[1], int(a[2]), float(a[3])] for a in fst_data]

        plt.figure(figsize=(12, 6))
        plt.xlabel("Chromosome")
        plt.ylabel("-log10(p-value)")
        plt.title(file)
        sig_thres = -1 * np.log10(0.05 / len(fst_data))
        pot_thres = -1 * np.log10(1 / len(fst_data))
        for chr in chromosome_offset:
            sub_data = [a for a in fst_data if a[0] == chr]
            xy = [[
        ...
        plt.savefig(f"../../data/304_exploratory_Gemma/fst-{file}.png")
        print()